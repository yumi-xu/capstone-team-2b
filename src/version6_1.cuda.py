...
df = cudf.read_csv('csv/incidents.all.csv')
...
fig = px.pie(values=df['Severity'].value_counts().to_arrow(), names=labels, width=500, height=400,
...
def text_to_tfidf(df, column):
    ...
    tfidf_df = cudf.DataFrame(tfidf_matrix.toarray(), columns=tfidf.get_feature_names_out())
...
tfidf_features_df = cudf.DataFrame()
column_names = []
...
for column in ['Title', 'Principles', 'Industries', 'Affected Stakeholders', 'Harm Types', 'Summary', 'Evidences',
    ...
    tfidf_features_df = cudf.concat([tfidf_features_df, tfidf_df], axis=1)
...
mi_scores = mutual_info_classif(X, y_encoded.to_numpy())
print("begin step 4: " + str(datetime.now()))
# Step 4: Feature Importance Ranking
# Aggregate importance scores for each original column
feature_importance_df = cudf.DataFrame({'Feature': column_names, 'Importance': mi_scores})
...
plt.barh(aggregated_importance['Feature'].head(top_feature_num).to_arrow(),
         aggregated_importance['Importance'].head(top_feature_num).to_arrow())
...
serialize_to_file(tfidf_features_df.to_pandas(), f'{tmpDir}/tfidf_features_df.pkl')
serialize_to_file(y.to_pandas(), f'{tmpDir}/y.pkl')
serialize_to_file(y_cat.to_pandas(), f'{tmpDir}/y_cat.pkl')
serialize_to_file(y_encoded.to_pandas(), f'{tmpDir}/y_encoded.pkl')
serialize_to_file(top_aggregated_importance.to_pandas(), f'{tmpDir}/top_aggregated_importance.pkl')
